...
export function Round2ResultsSection() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/boule-nights/${nightId}/round-2-matches`);
        if (!res.ok) return;
        ...
        console.error("Failed to load round 2 matches", e);
      }
    }
  ...
  async function saveResult(match: MatchItem) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Failed to save match result (round 2)", res.status, text);
        alert("Kunde inte spara resultatet för omgång 2. Försök igen.");
        return;
      }
      ...
      console.error("Failed to save match result (round 2)", e);
      alert("Tekniskt fel vid sparande av resultat för omgång 2.");
    } finally {
      setSavingId(null);
    }
  }
  ...
  return (
    <section className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 text-sm shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <header className="space-y-1">
        <h2 className="text-base font-medium">Resultat  omgång 2</h2>
        <p className="text-xs text-zinc-600 dark:text-zinc-400">
          Fyll i resultat för varje match i omgång 2. Poängen används senare för lottning av omgång 3.
        </p>
      </header>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    aria-label="Poäng för Lag A (omgång 2)"
                  />
                </div>
                <div className="flex items-center gap-2">
                  <span className="flex-1">
                  ...
                  <input
                    ...
                    className="w-16 rounded border border-zinc-300 bg-white px-1 py-0.5 text-xs outline-none ring-0 focus:border-zinc-500 focus:ring-2 focus:ring-zinc-500/40 dark:border-zinc-700 dark:bg-zinc-950"
                    aria-label="Poäng för Lag B (omgång 2)"
                  />
                </div>
              </div>
